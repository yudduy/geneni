...
class Harmonizome(ToolInterface):
    
    name: str = "Harmonizome"
    description: str = ( 
        "Use this to get diseases linking to this gene. It will return what disease are associated with a gene based on the given gene symbol or ID. "
        "Input: a valid gene symbol or ID (e.g. CFTR)"
    )
        
    __version__: float = VERSION
    DATASETS: dict = DATASET_TO_PATH.keys()
    ...
    def use(self, input_string: str):
        myenum = enum()
        myentity = Entity(myenum)
        mycls = input_string.strip(' ')
        return {"result": self.get(mycls, myentity)}
# Utility functions
# -------------------------------------------------------------------------
